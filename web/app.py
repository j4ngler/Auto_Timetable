...
def create_app() -> Flask:
    ...
    def run_script(job_id: str, script: str, args=None):
        if args is None:
            args = []
        script_abs = project_path(script)
    ...
    def job_run_scheduler(job_id: str, extra_args=None):
        ...
        if os.path.exists(project_path('run_pipeline.py')):
            return run_script(job_id, 'run_pipeline.py', args=(extra_args or []))
        elif os.path.exists(project_path('greedy_solver.py')):
    ...
    def job_run_recommender(job_id: str, extra_args=None):
        ...
        if os.path.exists(project_path('ai_recommender.py')):
    ...
    def job_run_recommend_schedule(job_id: str):
        ...
        if os.path.exists(project_path('recommend_schedule.py')):
        ...
        try:
            ...
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                # Hiển thị dạng bảng key/value cho JSON object
