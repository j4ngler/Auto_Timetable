...
SLOTS_CSV = Path('timeslots.csv')
CONSTRAINTS_JSON = Path('constraints.json')
OUTPUT_SCHEDULE = Path('schedule_final.csv')
...
def greedy_schedule(classes: pd.DataFrame, slots: pd.DataFrame, constraints: dict) -> pd.DataFrame:
    ...
    tt_path = Path('timetable_all.csv')
    if tt_path.exists():
        try:
...
def main():
    if not CLASSES_CSV.exists() or not SLOTS_CSV.exists() or not CONSTRAINTS_JSON.exists():
        print('Thieu file dau vao. Can classes_to_schedule.csv, timeslots.csv, constraints.json')
        return
    ...
    print(f'Da tao {OUTPUT_SCHEDULE.resolve()} ({len(schedule)} dong)')
